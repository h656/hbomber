...
def banner():
    ...
    clr()
    logo="""                                                  
d    d d ss.    sSSSs   d s   sb d ss.  d sss   d ss.  
S    S S    b  S     S  S  S S S S    b S       S    b 
S    S S    P S       S S   S  S S    P S       S    P 
S sSSS S sSS' S       S S      S S sSS' S sSSs  S sS'  
S    S S    b S       S S      S S    b S       S   S  
S    S S    P  S     S  S      S S    P S       S    S 
P    P P `SS    "sss"   P      P P `SS  P sSSss P    P          
                                         """
...
def infinite(pn, dl, ch, max):
    ...
    while True:
        ...
        count_inf += 1
        # os.system('echo Harsh >> count.xxx')
        time.sleep(float(dl))
        if (count_inf > maxlim):
            exit()
...
def start(target, counter, delay, ch, cc):
    ...
    while success < (int(bombs)):
        ...
        try:
            ...
            if cc == "91":
                print('Sorry All APIs Have Expired Please Update hbomber')
        ...
        print("              This Bomber Was Created By Harsh !!                ")
        print("==================================================================")
        try:
...
ver = urllib.request.urlopen(
    "https://raw.githubusercontent.com/h656/hbomber/master/.version").read().decode('utf-8')
verl = ''
try:
...
try:
    noti = urllib.request.urlopen(
        "https://raw.githubusercontent.com/h656/hbomber/master/.notify").read().decode('utf-8')
...
if nm == 0:
    ...
    if nt <= 0 or nt >= 30:
        print('\thbomber Shows Better Result in 10 to 25 Threads\n\t\tStill Continuing....')
